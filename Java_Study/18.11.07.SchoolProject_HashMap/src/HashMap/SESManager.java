...
public class SESManager {
	private Map<String, Human> humanMap; //관리인원(교수, 연수생, 운영진) 객체를 저장할 저장 공간
	
	/**
	 * 주어진 size의 초기값으로 새로운 SESManager 클래스의 객체를 생성한다.	 * 
	 * */
	public SESManager(){
		humanMap = new HashMap<String, Human>();
	}
	...
	public boolean insertHuman(Human human){
		...
		// key, value를 페어(pair)로 추가한다
		humanMap.put(human.getJumin(), human);
		return true;
	}
	...
	public Human findHuman(String jumin){
		if (humanMap.containsKey(jumin)) {
			// 일치되는 주민번호를 찾았다면 반환한다
			return humanMap.get(jumin);
		}
		return null;
	}
